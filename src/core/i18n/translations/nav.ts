export const nav = {
  ID: {
    navDashboard: "Dashboard",
    navLogout: "Keluar",
    navProfile: "Profil",
    navOpenMenu: "Buka menu",
    navCloseMenu: "Tutup menu",
    navMore: "Lainnya",
    menuBusiness: "Bisnis",
    menuKasir: "Kasir",
    menuMaster: "Master",
    menuProduk: "Produk",
    menuKategoriProduk: "Kategori Produk",
    menuSupplier: "Supplier",
    menuPenerimaanBarang: "Penerimaan Barang",
    menuRole: "Role",
    menuAkun: "Users",
    menuKalkulator: "Kalkulator",
    menuAnalytics: "Analitik",
    menuStok: "Stok",
  },
  EN: {
    navDashboard: "Dashboard",
    navLogout: "Sign Out",
    navProfile: "Profile",
    navOpenMenu: "Open menu",
    navCloseMenu: "Close menu",
    navMore: "More",
    menuBusiness: "Business",
    menuKasir: "Cashier",
    menuMaster: "Master",
    menuProduk: "Products",
    menuKategoriProduk: "Product Categories",
    menuSupplier: "Suppliers",
    menuPenerimaanBarang: "Stock Receipts",
    menuRole: "Roles",
    menuAkun: "Users",
    menuKalkulator: "Calculator",
    menuAnalytics: "Analytics",
    menuStok: "Stock",
  },
} as const;
